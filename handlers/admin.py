...
# 		response_wl = f'Added {data["nickname"]} to the whitelist'
...
# 		if await bot_rc.mcrcon_server_status():
# 			await other.other_source_UserAlert(
...
# 			response = ''
# 			try:
# 				player_rn = await bot_rc.mcrcon_admin_list_player()
# 			except:
# 				await other.other_source_UserAlert(
# 					id=query.from_user.id,
# 					type='server_offline',
...
# 				data_op = f'Made {data["nickname"]} a server operator'
...
# 						InlineKeyboardButton('Одобрить', callback_data=f'accept_application {id}'),
...
# 			message_response = ''
# 			if len(command) > 0:
# 				response = await bot_rc.mcrcon_universal(command=command)
...
# 	dp.register_callback_query_handler(admin_callback_ApplicationAccept,
# 	                                   lambda x: x.data.startswith('accept_application'))
# 	dp.register_callback_query_handler(admin_callback_ApplicationReject,
# 	                                   lambda x: x.data.startswith('reject_application'))
